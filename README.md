...
To submit your model's results, see [Contributing Results](#contributing-results).
...
Yes. Run the full benchmark at d=384, save the output JSON, and open a pull request with your result file; the steps are in [Contributing Results](#contributing-results).
...
CC-BY-NC-4.0 with an evaluation-only addendum. See [LICENSE](LICENSE) for the license summary and addendum text, and [creativecommons.org/licenses/by-nc/4.0/legalcode](https://creativecommons.org/licenses/by-nc/4.0/legalcode) for the full legal code.

As licensor, Latimal grants two permissions on top of the license, recorded in [LICENSE](LICENSE): running FoodEval to evaluate and compare models is permitted in any setting, including inside a company; and the evaluation code and tooling may be used commercially. What is never permitted, in any setting: using the data to train, fine-tune, or distill a model, or selling the data. The data may be redistributed only under these same terms, unmodified; the code carries the commercial-use permission above.
